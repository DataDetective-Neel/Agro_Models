# train_models.py - LightGBM Version
import pandas as pd
import numpy as np
import joblib
...
try:
    ...
    print("Please make sure 'data/raw/combined_dataset.csv' exists")
    exit()
...
print("\nTRAINING SOIL MOISTURE MODEL (LightGBM Regressor)...")
moisture_model = LGBMRegressor(
...
print("✅ Soil Moisture LightGBM Model trained")

# Train Crop Classification Model
print("\nTRAINING CROP CLASSIFICATION MODEL (LightGBM Classifier)...")
crop_model = LGBMClassifier(
    n_estimators=200,
    max_depth=8,
    learning_rate=0.1,
    num_leaves=31,
...
print("✅ Crop Classification LightGBM Model trained")
...
print(f"\nSOIL MOISTURE MODEL PERFORMANCE:")
...
print("\nSOIL MOISTURE FEATURE IMPORTANCE:")
...
# Save models
joblib.dump(moisture_model, 'ML models/soil_moisture_model.pkl')
joblib.dump(crop_model, 'ML models/crop_classifier_model.pkl')
joblib.dump(scaler_reg, 'ML models/scaler_reg.pkl')
joblib.dump(scaler_clf, 'ML models/scaler_clf.pkl')
joblib.dump(le, 'ML models/label_encoder.pkl')

# Save feature lists for reference
joblib.dump(moisture_features, 'ML models/moisture_features.pkl')
joblib.dump(crop_features, 'ML models/crop_features.pkl')

print("All LightGBM models and artifacts saved to 'ML models/' directory")
...
joblib.dump(model_info, 'ML models/model_metadata.pkl')
...
print("All models saved in 'ML models/' directory")
print("LightGBM offers FASTER training and prediction compared to XGBoost!")
